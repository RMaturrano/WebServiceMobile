...
try{
	...
	if (id !== undefined)
	{
        ...
        var query = 'select IFNULL("ClaveMovil",\'\') AS "ClaveMovil" , ' +
                        ...
                        '    IFNULL(UPPER("NombreRazonSocial"),\'\') AS "NombreRazonSocial" , ' +
    	...
    	if (rs.length > 0)
    	{
    		...
    		for(i = 0; i < rs.length ; i++)
    		{
        		...
        		businessPartner += '"NumeroDocumento": "'+rs[i].NumeroDocumento+'",';
        		businessPartner += '"NombreRazonSocial": "'+rs[i].NombreRazonSocial+'",';
        		businessPartner += '"NombreComercial": "'+rs[i].NombreComercial+'",';
        		...
        		businessPartner += '"Telefono1": "'+rs[i].Telefono1+'",';
        		businessPartner += '"Telefono2": "'+rs[i].Telefono2+'",';
        		businessPartner += '"TelefonoMovil": "'+rs[i].TelefonoMovil+'",';
        		businessPartner += '"CorreoElectronico": "'+rs[i].CorreoElectronico+'",';
        		...
        		if (rsDet.length > 0)
            	{
            	    for(j = 0; j < rsDet.length ; j++)
            		{
                		...
                		businessPartnerContacts += '"IdContacto": "' + rsDet[j].IdContacto + '",';
                		businessPartnerContacts += '"PrimerNombre": "' + rsDet[j].PrimerNombre + '",';
                		businessPartnerContacts += '"SegundoNombre": "' + rsDet[j].SegundoNombre + '",';
                		businessPartnerContacts += '"Apellidos": "' + rsDet[j].Apellidos + '",';
                		businessPartnerContacts += '"Posicion": "' + rsDet[j].Posicion + '",';
                		businessPartnerContacts += '"Direccion": "' + rsDet[j].Direccion + '",';
                		businessPartnerContacts += '"CorreoElectronico": "' + rsDet[j].CorreoElectronico + '",';
                		businessPartnerContacts += '"Telefono1": "' + rsDet[j].Telefono1 + '",';
                		businessPartnerContacts += '"Telefono2": "' + rsDet[j].Telefono2 + '",';
                		businessPartnerContacts += '"TelefonoMovil": "' + rsDet[j].TelefonoMovil + '"';
                		businessPartnerContacts += "}";
            	...
            	query = ' select  IFNULL("Tipo", \'\') AS "Tipo", ' + 
        		                ...
        		                ' IFNULL(UPPER("Calle"), \'\') AS "Calle", ' + 
        		                ' IFNULL(UPPER("Referencia"), \'\') AS "Referencia", ' + 
        		...
        		if (rsDet.length > 0)
            	{
            	    ...
            	    for(j = 0; j < rsDet.length ; j++)
            		{
                		...
                		businessPartnerDirections += '"Calle": "' + rsDet[j].Calle + '",';
                		businessPartnerDirections += '"Referencia": "' + rsDet[j].Referencia + '",';
